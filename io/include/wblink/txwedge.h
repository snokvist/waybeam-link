...
class TxWedge {
  public:
    ...
    // total — a craft that wedged briefly twice in a flight would eventually
    // trip a cumulative threshold with no fault present. Reset by any backend
    // TX progress; an idle window holds it, matching how the verdict itself
    // treats an idle window as evidence of nothing.
    uint32_t consecutive_wedged() const { return consecutive_; }
