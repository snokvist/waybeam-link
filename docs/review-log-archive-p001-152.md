> **ARCHIVE — frozen at Pass 152 (2026-08-06).** This is the complete Pass 1–152
> spec-review history, retired as a growing log on the operator's Pass-152
> correction: too much was being ruled while still being figured out. The live
> log continues in `review-log.md` under a two-tier format (contract rulings
> only, ≤40 lines per pass); measurement-phase results now go to `findings.md`.
> Nothing in this file is invalidated by the move — Pass numbers cited anywhere
> in the repo (`§`-comments, PR descriptions, coordination memory) resolve here.
...
**Pass 150 post-merge review — two CRITICALs, both self-inflicted (2026-08-06).**

1. **`is_radio()` is `is_rf()`, not "is devourer".** `set_backend_relative(air.value->is_radio())` therefore flagged **kernel-monitor as relative too** — the one backend where absolute qdb is correct. The §10.2 curve resolve and §10.6 calibration start were consequently refused on *every* real backend, making calibration unreachable fleet-wide, and the §10.2 contract ("a loaded `power_map` actuates on both RF backends") silently false. The gate now reads `cfg.air.kind == AirCfg::Kind::kRadio`. The claim in this log that the sweep was "devourer-exclusive because it is bound only when `is_radio()`" was wrong on the same misreading — recorded, not quietly corrected. `tests/app_test.cpp` asserted the intended split and passed throughout, because the test sets the flag directly: **a test that injects the very value production computes wrongly cannot catch this class of bug.**

2. **The ground's §10.7 measured absolute and applied relative.** The uplink sweep probes through `set_power_qdb` (absolute), so every value the owner precedence yields — artifact placement, `power_map` resolve, latch — is an absolute qdb. Converting only `upwr.apply_qdb` to the relative actuator meant an 84 qdb placement became `108 + 84 = 192 qdb` → `iw txpower fixed 4800`, silently clamped to regulatory max, with the return discarded. Reachable with no operator action: boot, any pairing-key change, run completion, `set_tier()`. Measurement and application must share a space, so the uplink path stays **absolute** until §10.7 is re-based alongside §10.6. The ground latch bound is reverted with it — bounding an absolute value by a relative bound is a category error, and the ground's §10.5 conversion is now explicitly deferred rather than half-applied.

The shape of both: Pass 150 converted the paths that were in mind and left the ones that were not, and the second review caught what the first fix introduced. A partial conversion of a two-space system is more dangerous than no conversion, because each half is individually defensible.
