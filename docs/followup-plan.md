...
| — | `229b` return-TX diagnosis | **[HISTORICAL — kernel-monitor] RESOLVED (Pass 48/49):** AF_PACKET `send()` succeeded 915 times while netdev TX counters did not advance and no RF aired. **The `tx_wedged`-from-netdev-TX-progress detector went with the backend.** What survives is the §9.10 principle — submission success is not TX liveness — now carried on devourer by CCX `tx_progress` and measured there directly in **PR #168** (induced wedge cleared 5/5 by in-process backend rebuild; do-nothing control 0/5; `docs/findings.md` 2026-08-10) |
| — | Independent ARQ cache | **[HISTORICAL RF, LIVE LOGIC — Pass 48]** real monitor cache + 150‰ N=1 aggregator stress reduced unrecoverable frames 534→119 (−77.7%) with zero rejected replies. Resend load stayed ~0.92/frame because cache + ARQ are parallel. The cache control path is **localhost** UDP/IP and is backend-independent; only the RF ears were monitor, and the stress loss itself was a deterministic synthetic post-radio drop |
| — | Cache timing/ordering evidence | **[HISTORICAL RF, LIVE SEED — Pass 50]** real P95 first reply/completion 2.845/2.910 ms; a targeted 3 ms first-NACK grace cut NACK packets 22.5% and vehicle resends 21.3% in clean 1,800-frame A/B runs. Default 3 ms, range 0..6, exact block + first NACK only. **The 3 ms seed measures a localhost UDP/IP round trip, not the air**, and the loss it was measured under was a deterministic synthetic 150‰ post-radio drop rather than RF loss — so it transfers; recorded here so the provenance is not lost |
| — | Controller/cache regression rerun | **DONE (Pass 52), backend-independent:** the archive records these as the **UDP controller gates**, not an RF run — both cache modes, cache-offload parity invariant, JSCC enforcement, FPS ladder, and the 158 s all-controller soak. Unaffected by the retirement |
...
| — | §11 CSA machinery on devourer | **OPEN — TIER 1, operator ruling (2026-08-10).** §11.6's craft post-retune RX-liveness guard (Pass 80) was characterised on the craft's in-place **`iw set freq`** retune — the kernel netdev path, retired in Pass 164. Devourer retunes via `FastRetune`/`SetMonitorChannel`, so it is unknown whether the half-retune failure the guard defends against can occur at all on the shipping backend. Separately §11.2's `dt_to_switch_ms` 150/500 ms seeds are **paper-derived against wfb_ng precedent and never TSF-measured** (`step11-bench.md` §4.3; `preflight-open-issues.md` C3 "never run"). A devourer CSA retune run answers both. **RULED (operator, 2026-08-10): run the trial** — queued as an RF leg in #134; §11 is not amended until it reports |
| — | Pass 51 root/`0666` premise | **OPEN (2026-08-10).** Pass 51 justified running as root and creating `/venc_frame_out` `0666` on the grounds that "the kernel-monitor backend requires raw packet access". Devourer uses libusb, not `AF_PACKET`, so the stated premise is void. Whether the privilege posture should change is a separate question — the audit only records that its reason no longer holds |
| — | §17 gate-2 transferability | **RULED TRANSFERABLE (operator, 2026-08-10).** The only physical measurement is Pass 47 on the deleted backend, but gate 2 measures cross-adapter loss *correlation* — a property of antenna geometry and the channel. The backend moves *absolute* loss, not the correlation between two co-located ears. Pass 139 was weighed and does not apply: it refuted a devourer **TX capability** claim, not a geometric property measured identically either way. The verdict stands and the 10% FEC seed is unchanged; provenance recorded at every citation site rather than re-flagged |
